...
pub fn trust_update(
    ...
    let next =
        trust + coefficients.lambda * successes.max(0.0) - coefficients.mu * failures.max(0.0);
    unit(next)
}
...
pub fn specialization_update(
    ...
    unit(
        specialization + coefficients.kappa * experience.max(0.0)
            - coefficients.obsolescence * obsolescence.max(0.0),
    )
}
...
mod tests {
    ...
    fn omega_is_bounded_below_at_zero_and_above_at_the_ceiling() {
        ...
        assert_eq!(
            global_intelligence(0.05, &collapse, &coefficients),
            OMEGA_FLOOR
        );
        let boom = OmegaDelta {
            learning: 1.0,
        ...
        assert_eq!(
            global_intelligence(OMEGA_CEILING, &boom, &coefficients),
            OMEGA_CEILING
        );
    }
    ...
    fn mesh_intelligence_splits_raw_compute_from_emergence() {
        ...
        assert!(
            together.total > alone.total,
            "connection must beat isolation"
        );
    ...
    fn mesh_intelligence_ignores_the_diagonal_and_extra_matrix_rows() {
        ...
        assert!(
            (bounded.emergent - 2.0).abs() < 1e-12,
            "only the first two rows apply"
        );
    }
    ...
    fn top_emergent_pairs_ranks_the_strongest_collaborations() {
        ...
        assert!(
            matches!(pairs[0], (0, 1, _) | (1, 0, _)),
            "the two strong agents lead"
        );
    }
    ...
    fn trust_grows_on_success_and_decays_on_failure() {
        ...
        assert!(decayed < 0.5);
        assert_eq!(
            trust_update(1.0, 100.0, 0.0, &coefficients),
            1.0,
            "trust saturates"
        );
        assert_eq!(
            trust_update(0.0, 0.0, 100.0, &coefficients),
            0.0,
            "trust floors"
        );
    }
    ...
    fn specialization_accretes_with_experience() {
        ...
        assert!(
            specialization > 0.3,
            "repeated domain work compounds mastery"
        );
    ...
    fn plasticity_follows_trust_but_pays_for_latency() {
        ...
        assert!(
            near > far,
            "a distant partner is a weaker connection at equal trust"
        );
    ...
    fn plasticity_settles_instead_of_pinning_at_one() {
        ...
        assert!(
            trusted > tolerated,
            "W must grade partners, not saturate for all of them"
        );
        assert!(
            tolerated < 0.95,
            "a merely tolerated link should not pin at full strength"
        );
    ...
    fn resource_efficiency_is_performance_per_cost() {
        ...
        assert!(
            resource_efficiency(0.8, 0.0, 0.0).is_finite(),
            "zero cost must not divide by zero"
        );
    }
    ...
    fn squash_maps_into_the_unit_interval() {
        ...
        assert!(
            (0.99..=1.0).contains(&squash(50.0)),
            "large input saturates high"
        );
        assert!(
            (0.0..0.01).contains(&squash(-50.0)),
            "large negative input saturates low"
        );
    ...
    fn deterministic_draws_are_reproducible_and_well_distributed() {
        assert_eq!(
            deterministic_unit(seed_of("task-1")),
            deterministic_unit(seed_of("task-1"))
        );
        assert_ne!(
            deterministic_unit(seed_of("task-1")),
            deterministic_unit(seed_of("task-2"))
        );

        let mean = (0..1000).map(deterministic_unit).sum::<f64>() / 1000.0;
        assert!(
            (mean - 0.5).abs() < 0.05,
            "draws should be roughly uniform, got mean {mean}"
        );
